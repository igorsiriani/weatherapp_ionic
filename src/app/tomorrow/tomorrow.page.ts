...
export class TomorrowPage implements OnInit {
  ...
  async getForecast(lat, long) {
    ...
    .subscribe((response: any) => {
      this.temp = Math.round(response['daily'][1]['temp']['day']).toString();

      var weatherTemp = response['daily'][1]['weather'][0]['description'];
      weatherTemp = weatherTemp.split(' ').map((str: string) => str[0].toUpperCase() + str.substring(1)).join(' ');
      
      this.weather = weatherTemp;

      let iconRq = response['daily'][1]['weather'][0]['icon'].substring(0, 2);
      iconRq = './assets/images/' + iconRq + '.png';
      this.icon = iconRq;

      this.min = Math.round(response['daily'][1]['temp']['min']).toString();
      this.max = Math.round(response['daily'][1]['temp']['max']).toString();
